...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    val build = mutableMapOf<Int, MutableList<String>>()
    for ((name, grade) in grades)
        build.getOrPut(grade) { mutableListOf() }.add(name)
    return build
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    for (one in list.indices) {
        for (two in 1 until list.size) {
            var res = 0
            res = list[one] + list[two]
            if (res == number && one != two) return Pair(one, two)
        }
    }
    return Pair(-1, -1)
}
